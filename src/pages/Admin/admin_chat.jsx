...
const AdminChat = () => {
  ...
  useEffect(() => {
    if (conversationsLoaded && conversations.length > 0 && location.state?.studentId) {
      const targetConversation = conversations.find(
        conv => conv.otherUserId === location.state.studentId
      );
      if (targetConversation) {
        setSelectedChat(targetConversation);
      }
    }
  }, [conversationsLoaded, conversations, location.state]);
